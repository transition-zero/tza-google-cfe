...
def run_hourly_matching_scenario(
    ...
    for bus in run['nodes_with_cfe_load']:
        ...
        for technology in configs['cfe_technology_palette'][run['palette']]:
            ...
            if technology in network.generators.type.unique():
                ...
                network.add(
                    ...
                    bus + '-' + technology + '-ext-' + str(params['build_year']) + '-hourly-matching', # generator name
            ...
            if technology in network.storage_units.carrier.unique():
                ...
                network.add(
                    'StorageUnit',
                    bus + '-' + technology + '-ext-' + str(params['build_year']) + '-hourly-matching',
                    bus=bus, 
        ...
        # ----
        # ADD CFE CONSTRAINT
        lhs_generators = [i for i in network.generators.index if 'hourly-matching' in i]
        lhs_storages = [i for i in network.storage_units.index if 'hourly-matching' in i]
        rhs_load = network.loads_t.p_set[bus + '-cfe-load']
        
        tza.constraints.constr_hourly_matching(
            network = network,
            lhs_generators = lhs_generators,
            lhs_storages = lhs_storages,
            rhs_load = rhs_load,
            sign = '>=',
